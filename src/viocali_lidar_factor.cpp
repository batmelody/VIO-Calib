...
bool RT::Evaluate(double const *const *parameters, double *residuals,
  ...
  residual = Rbl.transpose() * Rb0bk * (Rbl * lidar_point_ + tbl) +
             Rbl.transpose() * tb0bk - world_point_;

  if (jacobians) {
    if (jacobians[0]) {
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jacobian_Rb0bk(
          jacobians[0]);
      jacobian_Rb0bk.setZero();
      jacobian_Rb0bk = -Rbl.transpose() * Rb0bk *
                       Utility::skewSymmetric(Rbl * lidar_point_ + tbl);
    }
    if (jacobians[1]) {
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jacobian_tb0bk(
          jacobians[1]);
      jacobian_tb0bk.setZero();
      jacobian_tb0bk = Rbl.transpose() * Eigen::Matrix3d::Identity();
    }
  }
  return true;
}

bool EX::Evaluate(double const *const *parameters, double *residuals,
                  double **jacobians) const {
  ...
  // SE3 Tbl 3x4
  theta_bl << parameters[0][0], parameters[0][1], parameters[0][2];
  tbl << parameters[1][0], parameters[1][1], parameters[1][2];
  ...
  if (jacobians) {
    if (jacobians[0]) {
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jacobian_Rbl(
          jacobians[0]);
      jacobian_Rbl.setZero();
      jacobian_Rbl =
          -Rbl.transpose() * Rb0bk * Rbl *
              Utility::skewSymmetric(lidar_point_) +
          Utility::skewSymmetric(Rbl.transpose() * Rb0bk * Rbl * lidar_point_) +
          Utility::skewSymmetric(Rbl.transpose() * Rb0bk * tb0bk) +
          Utility::skewSymmetric(Rbl.transpose() * tb0bk);
    }
    if (jacobians[1]) {
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> jacobian_tbl(
          jacobians[1]);
      jacobian_tbl.setZero();
      jacobian_tbl = Rbl.transpose() * Rb0bk;
    }
  }
  return true;
}
